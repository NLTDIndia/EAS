...
<form action="{{ url('/upload_files') }}" id="fm_dropzone_main" enctype="multipart/form-data" method="POST" style="display:none">
    {{ csrf_field() }}
...
<div><a target ='new' href='<?php echo url("templates\\Default-Template.xlsx")?>'>Click here</a> to download the Performance Appraisal sample template.</div>
...
$(function () {
       ...
       // acceptedFiles: "image/*,application/pdf,application/xlsx",
        ...
        init: function() {
            ...
            this.on("success", function(file) {
                ...
                console.log(file);
                loadUploadedFiles();
                bootbox.alert('The file has been successfully uploaded.');
            });
    ...
    $("#EditFileModal #delFileBtn").on("click", function() {
        if(confirm("Delete "+$(".file-info-form input[name=filename]").val()+" ?")) {
            $.ajax({
...
function formatFile(upload) { 
    var image = '';
    if($.inArray(upload.extension, ["jpg", "jpeg", "png", "gif", "bmp", 'pdf']) > -1) {
 ...
 return '<li><a href="JavaScript:Void(0);" class="fm_file_sel" data-toggle="tooltip" data-placement="top" title="'+upload.name+'" upload=\''+JSON.stringify(upload)+'\'>'+upload.name+'</a></li>';
}
</script>
@endpush