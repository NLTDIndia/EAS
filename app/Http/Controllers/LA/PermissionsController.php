...
class PermissionsController extends Controller
{
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Permissions", "create")) {
			...
			return redirect()->route('permissions.index')->withSuccess('The permission details has been added successfully.');
	...
	public function update(Request $request, $id)
	{
		if(Module::hasAccess("Permissions", "edit")) {
			...
			return redirect()->route('permissions.index')->withSuccess('The permission details has been updated successfully.');
	...
	public function destroy($id)
	{
		if(Module::hasAccess("Permissions", "delete")) {
			...
			return redirect()->route('permissions.index')->withSuccess('The permission details has been deleted successfully.');
		} else {
		    return redirect('/dashboard');
		}
	}
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url('/permissions/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Permissions", "edit")) {
					$output .= '<a title="Edit" href="'.url('/permissions/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
				
				if(Module::hasAccess("Permissions", "delete")) {
					/* $output .= Form::open(['route' => ['.permissions.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
					$output .= ' <button class="btn btn-danger btn-xs btn-delete" type="button"><i class="fa fa-times"></i></button>';
					$output .= Form::close(); */
				}
	...
	public function save_permissions(Request $request, $id)
	{
		if(Entrust::hasRole('SUPER_ADMIN')) {
			...
			return redirect('/permissions/'.$id."#tab-access");
		} else {
		    return redirect('/dashboard');
		}
	}
