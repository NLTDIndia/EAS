		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url('/backups/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
			...
			if($this->show_action) {
				$output = '';
				$output .= '<a href="'.url('/downloadBackup/'.$data->data[$i][0]).'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-download"></i></a>';
