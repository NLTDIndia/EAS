...
class UploadsController extends Controller
{
    ...
    public function get_file($hash, $name)
    {
        ...
        if(!$upload->public && !isset(Auth::user()->context_id)) {
            return response()->json([
                'status' => "failure",
        ...
        if($upload->public || Entrust::hasRole('SUPER_ADMIN') || Auth::user()->context_id == $upload->user_id) {
				...
				//Checking whether the file is exists or not
				  if(file_exists(storage_path('uploads/'.$filename))) {
				      ...
				      rename(storage_path('uploads/'.$filename), storage_path('uploads/'.$newFileName) );
				        ...
				        DB::table('uploads')
				      ->where('name', $filename)
				      ->update(['name' => $newFileName , 'user_id' => $userId, 'updated_at' => date("Y-m-d H:i:s") ] ); 
				    //  echo "UPDATE uploads set path = REPLACE(path, $filename, $newFileName)";
				      DB::update("UPDATE uploads set path = REPLACE(path,'". $filename."','". $newFileName."')");
    ...
    public function update_caption()
    {
        if(Module::hasAccess("Uploads", "edit")) {
			...
			if(isset($upload->id)) {
			    if($upload->user_id == Auth::user()->context_id || Entrust::hasRole('SUPER_ADMIN')) {
    ...
    public function delete_file()
    {
        if(Module::hasAccess("Uploads", "delete")) {
			...
			$upload = Upload::find($file_id);
			$templateusuageCount = DB::table('departments')
                            			->select('id', 'name', 'template_name')
                            			->whereNull('deleted_at')
                            			->where('template_name','=',$file_id)
                            			 ->count();
			
               if($templateusuageCount > 0){
                return response()->json([
                    'status' => "failure",
                    'message' => "Error: Could not delete this template."
                ]);
            }
			if(isset($upload->id)) {
			    if($upload->user_id == Auth::user()->context_id || Entrust::hasRole('SUPER_ADMIN')) {
					...
					return response()->json([
						'status' => "success",
                        'message' => "File has been successfuly deleted."
					]);
