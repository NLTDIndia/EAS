...
class LAFormMaker1
{
	...
	**/
	public static function input($module, $field_name, $default_val = null, $required2 = null, $class = 'form-control', $params = [], $readonly = null)
	{
			...
			switch ($field_type->name) {
					...
					if(is_array($default_val_arr) && count($default_val_arr) > 0) {
						...
						foreach ($default_val_arr as $uploadId) {
							...
							if(isset($upload->id)) {
								$uploadIds[] = $upload->id;
								$fileImage = "";
								if(in_array($upload->extension, ["jpg", "png", "gif", "jpeg", 'xls'])) {
				...
				case 'Radio':
					...
					if(starts_with($popup_vals, "@")) {
						...
						foreach ($popup_vals as $key => $value) {
						    $sel = false; //echo $value; echo "<br>";echo $default_val;exit;
							if($default_val != "" && $default_val == $value) {
								$sel = true;
							}
							$out .= '<label>'.(Form::radio($field_name, $key, $sel, [],  $readonly)).' '.$value.' </label>';
						}
						$out .= '</div>';
						break;
					} else {
						...
						foreach ($popup_vals as $value) {
							...
							$out .= '<label>'.(Form::radio($field_name, $value, $sel, [],  $readonly)).' '.$value.' </label>';
						}
						$out .= '</div>';
						break;
					}
				...
				case 'Textarea':
				    if($readonly == 'true') {
				        ...
				        // ############### Remaining
				        $out .= '<div class="htmlbox" id="htmlbox_'.$field_name.'">'.$default_val.'</div>';
				        $out .= Form::hidden($field_name, $default_val, $params);
				    }
				    else {
    					$out .= '<label for="'.$field_name.'">'.$label.$required_ast.' :</label>';
	...
	**/
	public static function display($module, $field_name, $link = 'TRUE', $class = 'form-control')
	{
