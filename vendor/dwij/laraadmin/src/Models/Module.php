...
class Module extends Model
{
	...
	**/
	...
	public static function setDefaultRoleAccessFromParentRole($parent_role_id, $role_id) {
	   $now = date("Y-m-d H:i:s");
	   DB::insert("insert into role_module (role_id, module_id, acc_view, acc_create, acc_edit, acc_delete,  created_at, updated_at) select ? , module_id, acc_view, acc_create, acc_edit, acc_delete, ?, ? from role_module  where role_id=?", [$role_id, $now, $now, $parent_role_id]);
	   DB::insert("insert into role_module_fields (role_id, field_id, access,  created_at, updated_at) select ? , field_id, access, ?, ? from role_module_fields  where role_id=?", [$role_id, $now, $now, $parent_role_id]);
	   
	}
	
	public static function updateRoleAccessFromParentRole($module_id, $parent_role_id, $role_id) {
	    $now = date("Y-m-d H:i:s");
	    $module = Module::find($module_id);
	    $module = Module::get($module->name);
	  ...
	  //  Log::debug('Module:setDefaultRoleAccess ('.$module_id.', '.$role_id.', '.$access_type.')');
	    ...
	    $parent_module_perm = DB::table('role_module')->where('role_id', $parent_role_id)->where('module_id', $module_id)->first();
	    $module_perm = DB::table('role_module')->where('role_id', $role_id)->where('module_id', $module_id)->first();
	    
	    if(!isset($module_perm->id)) {
	        DB::insert("insert into role_module (role_id, module_id, acc_view, acc_create, acc_edit, acc_delete,  created_at, updated_at) select ? , module_id, acc_view, acc_create, acc_edit, acc_delete, ?, ? from role_module  where role_id=? and module_id=?", [$role_id, $now, $now, $parent_role_id, $module_id]);
	        
	    } else {
	        DB::table('role_module')->where('role_id', $role_id)->where('module_id', $module->id)->update(['acc_view' => $parent_module_perm->acc_view, 'acc_create' => $parent_module_perm->acc_create, 'acc_edit' =>  $parent_module_perm->acc_edit, 'acc_delete' => $parent_module_perm->acc_delete, 'updated_at' => $now]);
	    }
	    ...
	    foreach ($module->fields as $field) {
	        // find role field permission
	        $parent_module_perm = DB::table('role_module_fields')->where('role_id', $parent_role_id)->where('field_id', $field['id'])->first();
	        $field_perm = DB::table('role_module_fields')->where('role_id', $role_id)->where('field_id', $field['id'])->first();
	        
	        if(!isset($field_perm->id)) {
	            DB::insert('insert into role_module_fields (role_id, field_id, access, created_at, updated_at) values (?, ?, ?, ?, ?)', [$role_id, $field['id'], $parent_module_perm->access, $now, $now]);
	        } else {
	            DB::table('role_module_fields')->where('role_id', $role_id)->where('field_id', $field['id'])->update(['access' => $parent_module_perm->access,  'updated_at' => $now]);
	        }
	    }
	    
	    
	    /* DB::insert("insert into role_module (role_id, module_id, acc_view, acc_create, acc_edit, acc_delete,  created_at, updated_at) select ? , module_id, acc_view, acc_create, acc_edit, acc_delete, ?, ? from role_module  where role_id=?", [$role_id, $now, $now, $ParentRoleId]);
	    DB::insert("insert into role_module_fields (role_id, field_id, access,  created_at, updated_at) select ? , field_id, access, ?, ? from role_module_fields  where role_id=?", [$role_id, $now, $now, $ParentRoleId]);
	     */
	}
